...
|---|---|---|---|---|
| **Core** (`gitsail-domain`, `gitsail-application`, `gitsail-git`, `gitsail-protocol`, `gitsail-forge`) | `0.0.0`, pinned workspace-wide (ADR-021) | Defines and produces `SCHEMA_VERSION = 1` (`gitsail-protocol`); written to the wire only by the CLI (producer role) — no in-repository consumer since ADR-025 | Built + unit/integration-tested on Linux, Windows, and macOS via the `rust` CI matrix job (`docs/architecture/ci-policy.md`) | Delivered — EPIC-01–EPIC-08, EPIC-16–EPIC-19, EPIC-22–EPIC-23 Must stories are `closed`/`review` in Takumi (§3) |
...
| **VS Code extension** (`apps/vscode`) | `0.0.0` pinned in `package.json`; the packaged `.vsix` is stamped with the release tag at package time only, never committed | Not applicable — since ADR-025 (supersedes ADR-015) the extension reads Git directly in TypeScript (`apps/vscode/src/git/`) instead of parsing `gitsail-cli` JSON, so it consumes no `Envelope` and no `schemaVersion` at all (`protocol-compatibility.md`) | Packaged and tested on `ubuntu-latest` only — that is sufficient: the `.vsix` is platform-independent JS/TS **and self-contained**, with no per-OS native artifact to match. Its only external requirement is the user's own Git ≥ 2.31 on `PATH` | Delivered — T-259/US-126 in `review`; EPIC-14 and EPIC-15 are `closed`/`review`. **Not published** to the VS Code Marketplace or Open VSX (registered decision, ADR-023) — the `.vsix` attached to the GitHub Release is the official install path today |
